...
bool setupEmpty(void) { meapOfJunks.open(); return true; }
...
bool testMeap4(void) {
  ...
  for (int a=0;a<500;a++) {
    ...
    meapOfJunks.editTocks(i,rand()%0x100);
    ordered();
  }
  return true;
}
...
bool chompT(void) {
  ...
  Chomped res;
  setupEmpty();
  j = (Junk){0}; res = meapOfJunks.chomp(8, &j, 0);
