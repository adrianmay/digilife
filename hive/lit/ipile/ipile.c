...
static bool isInFreeList(Pilehead * ph, Ix i) {
  //TODO: WRITEME
  return false;
}
...
void freeInPile(Pilehead * ph, Ix i, void * ghost, int ghostlen) {
  ...
  if (ph->fro == BAD_INDEX) {
    ...
    ph->fro = i; // Only if this is the first do we mess with the out end
  }
  ph->frn += 1;
